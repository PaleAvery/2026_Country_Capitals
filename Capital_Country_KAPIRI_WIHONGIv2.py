...
class easy_game:
    ...
    def new_round(self):
        ...
        # made a variable to control the maximum unaffected length of a capital without being shrunk
        max_length =16
        # put capitals on buttons
        for count, item in enumerate(self.colour_button_ref):
            capital = self.round_colour_list[count][1]

            if len(capital) >= max_length:
...
class hard_game:
    ...
    def new_round(self):
        ...
        self.answer_entry.delete(0, END)
        self.answer_entry.focus()
        self.results_label.config(text="Use Enter or Submit to input answers",bg="green")
    ...
    def __init__(self, partner, all_stats_info):
        ...
        if not is_hard:
            ...
            if rounds_won != 0:
                ...
                success_rate = rounds_won / rounds_played * 100
                score = (rounds_won * 100)  -(50*hintsused)

            else:
                success_rate = 0
                score = 0 - (50*hintsused)
            print(f" {rounds_won} / {rounds_played} *100")
